...
const Store = module.exports = function (ctx) {
  ...
  this._snapLayers = new StringSet();
  this.ctx = ctx;
  this.sources = {
    hot: [],
    cold: []
...
/**
 * Adds a snap layer or array of layers to the store.
 * @param {Object} layerIds
 *
 * @return {Store} this
 */
Store.prototype.addSnapLayers = function (layerIds) {
  toDenseArray(layerIds).forEach(id => {
    if (this._snapLayers.has(id)) return;
    this._snapLayers.add(id);
  });
  return this;
};

/**
 * Deletes a snap layer or array of layers from the store.
 * @param {string | Array<string>} layerIds
 * @return {Store} this
 */
Store.prototype.deleteSnapLayers = function (layerIds) {
  toDenseArray(layerIds).forEach(id => {
    if (!this._snapLayers.has(id)) return;
    this._snapLayers.delete(id);
  });
  return this;
};
